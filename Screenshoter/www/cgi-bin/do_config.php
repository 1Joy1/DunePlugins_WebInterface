...
class HD
{
	public static function http_get_document($url, $opts = null) {
		$ch = curl_init();
		if (self::get_item('proxy') !=''){
			$site_proxytype = (self::get_item('site_proxytype') !='') ? self::get_item('site_proxytype') : 0;
			if ((($site_proxytype == 0)&&(!preg_match("|kinopoisk|",$url)))
				||(($site_proxytype == 1)&&(preg_match("|kinopoisk|",$url)))
				||(($site_proxytype == 2)&&(preg_match("/kinopoisk|yandex/",$url)))
				||(($site_proxytype == 3)&&(preg_match("/kinokong/",$url)))){
				curl_setopt($ch, CURLOPT_PROXY,				self::get_item('proxy'));
				if (self::get_item('curlopt_proxytype') == 'CURLPROXY_SOCKS5')
					curl_setopt($ch, CURLOPT_PROXYTYPE,			CURLPROXY_SOCKS5);
				if (self::get_item('proxy_akk') !='')
					curl_setopt($ch, CURLOPT_PROXYUSERPWD,	self::get_item('proxy_akk'));
	...
	public static function save_items_tmp($path, $items) {
		file_put_contents (DuneSystem::$properties['tmp_dir_path']. '/' .$path, json_encode ($items));
	}
	...
	public static function get_items_tmp($path) {
		...
		if (file_exists($data_path))
			$items = json_decode(file_get_contents($data_path));
		return $items;
	}
