...
class smbtree
{
  ...
  public static function get_mount_smb ($ip_smb)
  {
	...
	foreach ($ip_smb as $k => $vel){
		...
		if (isset($df_smb[$k])){
			...
			if ((isset($vel['user']))&&(isset($vel['password']))&&($vel['password'] != ''))
  ...
  public function get_smb_infs ($item=false)
  {
	...
	if (file_exists($path))
		...
		$data = fopen($path,"w");
		if (!$data)
		return ActionFactory::show_title_dialog('%tr%t5');
		fwrite($data, $item);
		@fclose($data);
		$q =  $item;
	}
 ...
 public function get_mount_all_smb ()
  ...
  {
	$select_folder = false;
	if ((!is_array($param))&&(file_exists(DuneSystem::$properties['data_dir_path'] .'/'. $param)))
		$save_folder = unserialize(file_get_contents(DuneSystem::$properties['data_dir_path'] .'/'. $param));
	else
		$save_folder = $param;
	if (is_array($save_folder)){
