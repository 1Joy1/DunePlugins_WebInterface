...
$(function() {
	...
	*/
	function createLi(id, note, r_path) {
		var note = $('<span class="note" id="n_' + id + '">' + note + '</span>').editable('update.php', editableOptions);
		var link = $('<a href="#"><img src="'+r_path+'img/delete.png" alt="Удалить" /></a>').click(removeLi);
		...
		if (note != '') {
			...
			$.post('addnew.php', {'notetext':note}, function(data) {
				...
				if (response['err'] != null) {
					...
					createLi(response['id'], response['note'], response['r_path']).prependTo('#sortable');
				}
			});
		} else {
